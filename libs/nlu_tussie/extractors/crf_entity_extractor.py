...
from nlu_tussie.utils import write_to_file
from nlu_tussie.models import nerEvalCounter, report_epoch_score_matr

from nlu_tussie.config import RasaNLUModelConfig, InvalidConfigError
...
class CRFEntityExtractor(EntityExtractor):
    ...
    @staticmethod
    def _convert_bilou_to_bioes(labels: List[Text]):
    ...
    def train(self, training_data, config, **kwargs):
        ...
        if train_examples.entity_examples:
            ...
            # without annotations
            dataset = self._create_dataset(filtered_entity_examples + filtered_dev_entity_examples)
            self._train_model(dataset[:train_examples.size()])
            
            # evaluation
            self.eval_res = {self.component_config.get("max_iterations", 0): self._eval_model(dataset[train_examples.size():])}
            

    def _eval_model(self, df_dev: List[List[Tuple[Text, Text, Text, Text]]]):
        """pass"""
        if self.ent_tagger is None:
            return {}

        if not df_dev:
            logger.warning(f"No dev dataset feed into {self.name} procedure")
            return {}

        x_dev = [self._sentence_to_features(sent) for sent in df_dev]
        y_dev = [self._sentence_to_labels(sent) for sent in df_dev]
        
        y_pred = self.ent_tagger.predict(x_dev)
        if self.component_config["BILOU_flag"]:
            y_dev = [self._convert_bilou_to_bioes(x) for x in y_dev]
            y_pred = [self._convert_bilou_to_bioes(x) for x in y_pred]

        eval_op = nerEvalCounter()
        return eval_op.run(y_dev, y_pred)
    ...
    def _convert_bilou_tagging_to_entity_result(self, tokens, entities):
        ...
        while word_idx < len(tokens):
            ...
            if end_idx is not None and float(confidence) >= self.component_config.get("confidence_threshold", 0.7):
    ...
    def _convert_simple_tagging_to_entity_result(self, tokens, entities):
        ...
        for word_idx in range(len(tokens)):
            ...
            if entity_label != 'O' and float(confidence) >= self.component_config.get("confidence_threshold", 0.7):
                if self.pos_features:
                    start = word.idx
    ...
    def persist(self, model_dir):
        ...
        if self.ent_tagger:
            write_to_file(os.path.join(model_dir, f"eval_{self.name}.txt"),
