"""
Usage: SVM auto-cv-parameters-tuning classifiers
"""
...
import numpy as np
from nlu_tussie.models import report_epoch_score_matr, clsEvalCounter

from nlu_tussie import utils
from nlu_tussie.classifiers import INTENT_RANKING_LENGTH
...
class SklearnIntentClassifier(Component):
    ...
    def train(self, training_data, cfg, **kwargs):
        ...
        if len(set(labels)) < 2:
            ...
            if X.shape[0] > 10000:
                ...
                folds = 1.
            eval_X = X[:int(X.shape[0] * folds)]
            eval_y = self.transform_labels_num2str(y[:int(X.shape[0] * folds)])
            pred_eval_y = self.transform_labels_num2str(self.clf.best_estimator_.predict(eval_X, eval_y))
            self.eval_res = {0: clsEvalCounter().run(eval_y, pred_eval_y)}
    ...
    def persist(self, model_dir):
        ...
        utils.write_to_file(os.path.join(model_dir, f"eval_{self.name}.txt"),
