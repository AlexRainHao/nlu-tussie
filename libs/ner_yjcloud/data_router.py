...
from ner_yjcloud import utils, config
from ner_yjcloud.components import ComponentBuilder
from ner_yjcloud.config import RasaNLUModelConfig
from ner_yjcloud.evaluate import get_evaluation_metrics, clean_intent_labels
from ner_yjcloud.model import InvalidProjectError
from ner_yjcloud.project import Project
from ner_yjcloud.train import do_train_in_worker, TrainingException
from ner_yjcloud.training_data.loading import load_data
from twisted.internet import reactor
from twisted.internet.defer import Deferred
from twisted.logger import jsonFileLogObserver, Logger
from ner_yjcloud.serverlog import nonJsonFileLogObserver
...
class DataRouter(object):
    ...
    def _create_query_logger(response_log):
        ...
        if response_log:
            ...
            log_file_name = "nerServer-{}-{}.log".format(timestamp,
                                                            os.getpid())
            ...
            query_logger = Logger(
                observer = nonJsonFileLogObserver(out_file, recordSeparator = ''),
                namespace = 'query-logger')
    ...
    def _list_projects_in_cloud(self):
        try:
            from ner_yjcloud.persistor import get_persistor
    ...
    def _create_emulator(mode):
        ...
        if mode is None:
            from ner_yjcloud.emulators import NoEmulator
            return NoEmulator()
        elif mode.lower() == 'wit':
            from ner_yjcloud.emulators.wit import WitEmulator
            return WitEmulator()
        elif mode.lower() == 'luis':
            from ner_yjcloud.emulators.luis import LUISEmulator
            return LUISEmulator()
        elif mode.lower() == 'dialogflow':
            from ner_yjcloud.emulators.dialogflow import DialogflowEmulator
    ...
    def _tf_in_pipeline(model_config):
        ...
        from ner_yjcloud.classifiers.embedding_intent_classifier import \
    ...
    def parse(self, data):
        ...
        time = data.get('time')
        response = self.project_store[project].parse(data['text'],
                                                     time,
                                                     model)
