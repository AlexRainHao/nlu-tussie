...
from ner_yjcloud.utils import lazyproperty, write_to_file
from ner_yjcloud.utils import list_to_str
from ner_yjcloud.training_data.util import check_duplicate_synonym
...
class TrainingData(object):
    ...
    def __init__(self,
        ...
        self.regex_intent = regex_intent if regex_intent else []
        self.regex_intent = self.normalize_regex_intents()
    ...
    def merge(self, *others):
        ...
        regex_intent = deepcopy(self.regex_intent)
    ...
    def normalize_regex_intents(self):
        """pass"""

        if not self.regex_intent:
            return self.regex_intent

        res = {}

        for case in self.regex_intent:
            intent = case.get("intent")
            regex = case.get("regex")

            for rgx in regex:
                res.update({rgx: intent})
    ...
    def as_json(self, **kwargs):
        ...
        from ner_yjcloud.training_data.formats import RasaWriter
        return RasaWriter().dumps(self)

    def as_markdown(self):
        # type: () -> str
        """Generates the markdown representation of the TrainingData."""
        from ner_yjcloud.training_data.formats import MarkdownWriter
